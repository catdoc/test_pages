...
CANNON.Demo = function(options){
    ...
    function init() {
        ...
        world.dispatcher().addEventListener("postStep",function(evt) {
            for(var label in world.profile)
...
CANNON.Demo.prototype.shape2mesh = function(body){
    ...
    for (var l = 0; l < body.shapesLength(); l++) {
        var shape = body.getShape(l);
        ...
        case CANNON.Shape.types.SPHERE:
            shape = CANNON.castSphereShape(shape);
            //console.warn('shape.radius ' + shape.radius);
            var sphere_geometry = new THREE.SphereGeometry( shape.radius, 8, 8);
            mesh = new THREE.Mesh( sphere_geometry, this.currentMaterial );
            break;

        case CANNON.Shape.types.PARTICLE:
            shape = CANNON.castParticleShape(shape);
        ...
        case CANNON.Shape.types.PLANE:
            shape = CANNON.castPlaneShape(shape);
        ...
        case CANNON.Shape.types.BOX:
            ...
            shape = CANNON.castBoxShape(shape); //CANNON.castObject(shape, CANNON._NativeBox); 
            //CANNON.dump(shape);

            var halfExtents = shape.halfExtents;
            var box_geometry = new THREE.BoxGeometry(  halfExtents.x*2,
                                                        halfExtents.y*2,
                                                        halfExtents.z*2 );
        ...
        case CANNON.Shape.types.CONVEXPOLYHEDRON:
            // TODO: if shape is Cylinder, is it ok ?
            shape = CANNON.castCylinderShape(shape);
            if (!shape) {
                shape = CANNON.castConvexPolyHedronShape(shape);
            }
        ...
        case CANNON.Shape.types.HEIGHTFIELD:
            shape = CANNON.castHeightfieldShape(shape);
            ...
            for (var xi = 0; xi < shape.getFirstArraySize() - 1; xi++) {
                var secondArraySize = shape.getSecondArraySize(xi) - 1;
                //console.warn('hf xi = ' + xi + ' , ' + secondArraySize);

                for (var yi = 0; yi < secondArraySize; yi++) {
                  //  console.warn('hf yi = ' + yi);
                    for (var k = 0; k < 2; k++) {
                        ...
                        //console.warn('v0 ' + v0.x + ',' + v0.y + ',' + v0.z);
                        //console.warn('v1 ' + v1.x + ',' + v1.y + ',' + v1.z);
                        //console.warn('v2 ' + v2.x + ',' + v2.y + ',' + v2.z);
        ...
        case CANNON.Shape.types.TRIMESH:
            shape = CANNON.castTrimeshShape(shape);
        ...
        var o = body.getShapeOffset(l);
        var q = body.getShapeOrientation(l);
